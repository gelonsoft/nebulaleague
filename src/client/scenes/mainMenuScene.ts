...
export class MainMenuScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.menuHTML.getChildByID('resumeButton').addEventListener(
            'click',
            (event: Event) => this.scene.sleep('mainMenuScene') 
        )

        this.menuHTML.getChildByID('exitButton').addEventListener(
            'click',
            (event: Event) => {
